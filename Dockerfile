...
COPY pyproject.toml README.md uv.lock /build/signaldesk-alert-rule-worker/
COPY src /build/signaldesk-alert-rule-worker/src
RUN python -m pip install uv==0.11.31 \
    && cd /build/signaldesk-alert-rule-worker \
    && UV_PROJECT_ENVIRONMENT=/app/.venv uv sync --locked --no-dev --no-editable \
    && rm -rf /build
USER 10001:10001
CMD ["signaldesk-alert-rule-worker"]
